...
TestRunnerResult  TestRunner::run(const std::string& executable_path,const TestSuite& testsuite){
    TestRunnerResult  runner_result;
    for(std::size_t i=0; i<testsuite.size();++i){
        const TestCase& test_case = testsuite.getTestCase(i);

        ExecutionResult execution_result = runner.run(executable_path,{},test_case.input);
